  ...
  content title: proc{ I18n.t("active_admin.dashboard") } do
    ...
    if current_user.role? :Root
        ...
        column do
          panel "Pending Payment" do
            table_for Payment.where("status != 'ack'").limit(20) do
              ...
              column ("Created At") { |p| p.created_at.strftime("%Y-%m-%d %H:%M:%S") }
            end
          end
        end
      end
      ...
      columns do
        column do
          panel "Recent Changes" do
            table_for PaperTrail::Version.where("whodunnit.email in (select email from user where id in (select user_id from user_companies where company_id = #{current_user.current_company.id}))").limit(20) do # Use PaperTrail::Version if this throws an error
              column ("Item") { |v| v.item } #, [:admin, v.item] }
              column ("Type") { |v| v.item_type.underscore.humanize }
              column ("Event") { |v| v.event }
              column ("Modified At") { |v| v.created_at.to_s :long }
